...
class DataParser:
    ...
    def parse_target_sheet(self, df_target: pd.DataFrame) -> pd.DataFrame:
        ...
        df['Номенклатура_норм'] = df.iloc[:, 18].astype(str).apply(normalize_nomenclature)  # Столбец S - индекс 18
    ...
    def parse_source_sheet(self, df_source: pd.DataFrame) -> pd.DataFrame:
        ...
        df['Номенклатура_норм'] = df.iloc[:, 8].astype(str).apply(normalize_nomenclature)  # Столбец I - индекс 8
        ...
        df['Контрагент_норм'] = df.iloc[:, 5].astype(str).apply(normalize_nomenclature)  # Столбец F - индекс 5
