"""ExcelCostCalculator - Main Entry Point (ИСПРАВЛЕННАЯ ВЕРСИЯ)"""
import argparse
import time
import gc
import pandas as pd
...
def process_chunk(df_chunk, engine, chunk_start_idx):
    """Обрабатывает chunk строк с улучшенной логикой"""
    success_count = 0
    error_count = 0
    ...
    for idx in range(len(df_chunk)):
        ...
        # Нормализация номенклатуры (УЛУЧШЕНИЕ)
        if pd.notna(nomenclature):
        ...
        if not acquisition_date:
            logger.debug(f"Строка {excel_row}: Дата не найдена в '{str(document_text)[:50]}'")
        ...
        if aq is not None:
            ...
            success_count += 1
            logger.debug(f"Строка {excel_row}: ✅ Успешно | {nomenclature[:30]} | {quarter} | AQ={aq}")
        else:
            logger.debug(f"Строка {excel_row}: ❌ Нет совпадений | {nomenclature[:30]} | {quarter}")
            df_chunk.at[df_chunk.index[idx], 'AQ_Стоимость_закупки'] = ERROR_CODES['MANUAL_CHECK']
            df_chunk.at[df_chunk.index[idx], 'AR_Прямая_СС'] = ERROR_CODES['MANUAL_CHECK']
            df_chunk.at[df_chunk.index[idx], 'AS_НР'] = ERROR_CODES['MANUAL_CHECK']
            error_count += 1
...
def main():
    global logger
    parser = argparse.ArgumentParser(description='ExcelCostCalculator v2 - Fixed')
    ...
    logger.info("="*60)
    logger.info("ExcelCostCalculator v2 - ИСПРАВЛЕННАЯ ВЕРСИЯ")
    logger.info("="*60)
