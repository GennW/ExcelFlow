...
Создать три новых столбца в конце таблицы вкладки **"СК ТПХ_1 пг"**:

- **Столбец 1: "Дата приобретения"** — извлеченная и нормализованная дата из документа приобретения
- **Столбец 2: "Квартал приобретения"** — определённый на основе даты квартал (1, 2, 3 или 4)
- **Столбец 3: "Рассчитанная себестоимость"** — значение из справочной вкладки или маркер ошибки
...
1. **Поиск по коду номенклатуры:**
   - Использовать регулярное выражение для поиска уникальных кодов в названии (например: `ОНГ\.216\.00\.000-01-032` или `МШГРП\.114\.015`)
...
4. **Извлечение кода номенклатуры (если доступен):**
   - Использовать регулярное выражение: `r'([А-ЯЁ]+\.?\d+\.?\d+\.?\d+(?:-\d+)?)'` для кириллицы
...
Исходная структура сохраняется полностью. **В конец таблицы (после последнего столбца) добавляются три новых столбца:**

| Столбец | Название | Описание | Формат |
|---------|----------|---------|--------|
| Новый +1 | Дата приобретения | Извлеченная из документа дата | DD.MM.YYYY или "*Дата не найдена*" |
| Новый +2 | Квартал приобретения | Квартал, определённый на основе даты | Q1, Q2, Q3, Q4 или пусто |
| Новый +3 | Рассчитанная себестоимость | Значение из справочника или метка ошибки | Число (2 десятичных знака) или "*ТРЕБУЕТ РУЧНОЙ ПРОВЕРКИ*" |
...
- Квартал приобретения: Q1
- Рассчитанная себестоимость: 15000.50
...
- Квартал приобретения: Q2
- Рассчитанная себестоимость: 8250.75
...
- Рассчитанная себестоимость: *ТРЕБУЕТ РУЧНОЙ ПРОВЕРКИ*
...
2. Определение квартала: Q1 (февраль)
...
5. Получено значение: 12750.25

Выходные данные:
- Дата приобретения: 15.02.2025
- Квартал приобретения: Q1
- Рассчитанная себестоимость: 12750.25
```
...
2. Определение квартала по дате реализации: Q1 (март)
3. Извлечение кода: "ЦМВ.32.00.000-03.4.2023"
4. Поиск в справочнике по коду + контрагенту + периоду: найдено совпадение
5. Получено значение: 9500.00

Выходные данные:
- Дата приобретения: *Дата не найдена*
- Квартал приобретения: Q1
- Рассчитанная себестоимость: 9500.00
```
...
Документ актуален на: **13 ноября 2025 года**
