"""Скрипт для проверки названий столбцов в Excel-файле"""
import pandas as pd
from pathlib import Path

def check_column_names(file_path, sheet_name):
    """Проверяет названия столбцов в указанном листе Excel-файла"""
    df = pd.read_excel(file_path, sheet_name=sheet_name)
    
    print(f"Файл: {file_path}")
    print(f"Лист: {sheet_name}")
    print(f"Всего столбцов: {len(df.columns)}")
    print("\nНазвания столбцов с индексами:")
    
    # Выведем названия столбцов с AO до AS (индексы 40-44)
...
if __name__ == "__main__":
    ...
    check_column_names(file_path, sheet_name)